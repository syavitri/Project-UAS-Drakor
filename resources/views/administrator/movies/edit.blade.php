            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        @endif
                        <form class="forms-sample" action="/administrator/movies/{{$movie->id}}" method="post">
                            @csrf
                            @method('PUT')
                                    ...
                                    <select class="form-control" id="genres_id" name="genres_id">
                                        @foreach ($genres as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $item->id == $movie->genres_id ? 'selected' : '' }}>{{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <textarea type="text" class="form-control" id="synopsis" placeholder="Synopsis" name="synopsis"
                                        value="{{ $movie->synopsis }}">{{ $movie->synopsis }}</textarea>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <select class="form-control" id="countries_id" name="countries_id">
                                        @foreach ($countries as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $item->id == $movie->countries_id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
